...
public class CMClient {
    ...
    static byte[] getResponse(HttpEntity entity) {
        byte[] result = null;
        try {
    ...
    public byte[] getTile(double latitude, double longitude, int zoom,
        ...
        return getTile(tilenums[0], tilenums[1], zoom, styleId, size);
    }
    
    /**
     * Get tile with given tileX, tileY, zoom, styleId and size
     * as a bytes of the tile image (PNG file)
     * 
     * @param tileX X-coordinate of requested tile
     * @param tileY Y-coordinate of requested tile
     * @param zoom zoom level, on which tile is being requested (0-18)
     * @param styleId style id of the requested tile
     * @param size length of requested tiles' sides (64 or 256)
     * @return tile image (PNG file) as byte array
     */
    public byte[] getTile(int tileX, int tileY, int zoom,
            int styleId, int size) {
        return callService(String.format("/%s/%s/%s/%s/%s.png", styleId, size, zoom,
                tileX, tileY), "tile", null);
    }
    ...
    public GeoResults find_range(String object_type, Point point, int results, int distance) throws ObjectNotFoundException{
        byte[] response = {};
        try {
        String uri = String.format("/geocoding/v2/find.js?object_type=%s&around=%s&results=%s&distance=%s", 
                URLEncoder.encode(object_type, "UTF-8"), point.toString(), URLEncoder.encode(Integer.toString(results), "UTF-8"),
                URLEncoder.encode(Integer.toString(distance), "UTF-8"));
