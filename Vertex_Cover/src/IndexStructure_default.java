...
//				System.out.println(source+"  "+dest);
		...
		System.out.println("Created graph from file");
		br.close();
		return g;
	}
	...
	public static void createStructure() throws IOException{
		...
		Set<Integer> vertex_cover = Streaming_VertexCover.compute_disk ("USA-road-d.CAL.gr");  //vertex cover
		System.out.println("Iteration 0 : "+vertex_cover.size());

		SimpleGraph<Integer, DefaultWeightedEdge> g = createGraphfromFile("USA-road-d.CAL.gr");
...
/*		while(itt1.hasNext()) {
		...
		*/
		...
		int init_size = 0;
		int curr_size = vertex_cover.size();
		int count = 1;
		int k = 3;
...
//			Set <Integer>vertex_cover1 = Streaming_VertexCover.VertexCoverAlgo_List(distance_g, sorted_vertex);
			Set<Integer> vertex_cover1 = null;
			if (count%2 != 0) {
				vertex_cover1 = VertexCovers.find2ApproximationCover(distance_g);
			}
			else {
				vertex_cover1 = Streaming_VertexCover.VertexCoverAlgo(distance_g, sorted_vertex);
			}
			//System.out.println("With the available algo "+answer.size());

			vertex_cover.addAll(vertex_cover1);
			curr_size = vertex_cover.size();
			System.out.println("Iteration "+count+" : "+vertex_cover.size()+"  "+(((float)vertex_cover.size())/((float)init_size)));
			++count;
			...
			distance_g = (SimpleGraph<Integer, DefaultWeightedEdge>)distance_g1.clone();
			//printtoFile(distance_g, k);
			++k;
	...
	public static void printtoFile(SimpleGraph<Integer, DefaultWeightedEdge> g, int method) throws IOException{
		...
		while(it.hasNext()) {
			...
			for (int i=0; i<list.size(); i++) {
				...
				out.write("\t");
				DefaultWeightedEdge e = g.getEdge(source, list.get(i));
				out.write(String.valueOf(g.getEdgeWeight(e)));
				out.write("\n");
			}
		}
