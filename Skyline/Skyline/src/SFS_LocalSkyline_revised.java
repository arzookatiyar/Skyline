...
class SFS_LocalSkyline_revised {
    ArrayList<Tuple> data = new ArrayList<Tuple>();
    static TreeSet<Tuple> sortedData=new TreeSet<Tuple>(new LocalComp1());
	...
	while (it1.hasNext()) {
	    ...
	    if(SFS_Algorithm.is_skyline_revised(test, skyline)) {
		skyline.add(test);
	    }
	    else
...
class LocalComp1 implements Comparator{
    public int compare(Object first, Object second) {
	Tuple A = (Tuple)first;
	Tuple B = (Tuple)second;
	
	double entropy_A = A.distance+(1/A.probability);
	double entropy_B = B.distance+(1/B.probability);
